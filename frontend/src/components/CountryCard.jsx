...
export default function CountryCard({ country, onClick }) {
  ...
  return (
    <div
      ...
      className={`group relative cursor-pointer bg-gray-800/70 backdrop-blur-sm border border-gray-700/50 rounded-xl p-6 shadow-lg ${
        !isMobile ? 'hover:shadow-xl hover:bg-gray-700/60 transition-all duration-300 hover:-translate-y-1' : ''
      } ${isMobile ? 'h-[380px]' : 'h-[420px]'} flex flex-col`}
    >
      {isFavorite && (
      ...
      <div className="relative z-10">
        <h3 className={`${isMobile ? 'text-xl' : 'text-2xl'} font-bold font-serif text-blue-400 mb-1 ${
          !isMobile ? 'group-hover:text-blue-300 transition-colors' : ''
